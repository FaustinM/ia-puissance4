...
def runGame(remoteCon, gamesCon):
    game = JeuPuissance4()
    isFinish = False
    currentPlayer = 1
    while not isFinish:
        if currentPlayer == 1:
            ...
            remoteCon.send(("predict", game))
            prediction = remoteCon.recv()
            result = game.play(prediction)
            currentPlayer = result[0]
            isFinish = result[1]

    remoteCon.send(("reward", isFinish, game))
    reward = remoteCon.recv()
    remoteCon.close()
    gamesCon.put(
        {
            "grid": game.grid,
...
if __name__ == "__main__":
    ...
    remoteCons_p = []
    gamesCon = multiprocessing.Queue()
    max_thread = multiprocessing.cpu_count()

    while i < trainCycles:
        if len(remoteCons_p) < max_thread and i + len(remoteCons_p) < trainCycles:
            remoteCon_p, remoteCon = multiprocessing.Pipe()
            p = Process(target=runGame, args=(remoteCon, gamesCon))
            p.start()
            remoteCons_p.append(remoteCon_p)

        for remoteCon_p in multiprocessing.connection.wait(remoteCons_p):
            if remoteCon_p.poll(0):
                m = remoteCon_p.recv()
                if m[0] == "reward":
                    remoteCon_p.send(ia.generateReward(m[1], m[2]))
                    remoteCons_p.remove(remoteCon_p)
                    i = i + 1
                    if i % 100 == 0:
                        ia.q.save(ia_name)
                    ...
                    remoteCon_p.send(ia.predict(m[1]))

        while not gamesCon.empty():
            lastGameSet.append(gamesCon.get())
